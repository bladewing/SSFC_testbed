...
class RestStatsApi(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ids_flow_hard_timeout = 9
        self.ids_flow_priority = 110
    ...
    def add_flow(self, datapath, match, actions, hard_timeout, priority, buffer_id=None):
        ...
        if buffer_id:
            mod = parser.OFPFlowMod(datapath=datapath, buffer_id=buffer_id, hard_timeout=hard_timeout, priority=priority, match=match, instructions=inst)
        else:
    ...
    def _packet_in_handler(self, ev):
        ...
        if ipv4_msg == None:
            self.logger.info("Ethernet-only packet -> classic output")
            # Classic output
        ...
        if tcp_msg == None:
            ...
            if msg.buffer_id == ofproto.OFP_NO_BUFFER:
                data = msg.data
            self.logger.info("Buffer_id:%s Max:%s", msg.buffer_id, ofproto.OFP_NO_BUFFER)
            out = ofp_parser.OFPPacketOut(datapath=datapath, buffer_id=msg.buffer_id, in_port=in_port, actions=actions, data=data)
            datapath.send_msg(out)
            return
